<template>
  <div>
    <el-button type="primary" size="small" @click="toAddHandler">添加员工</el-button>
    <el-input
      v-model="list.status"
      clearable
      placeholder="请输入状态"
    ...
    <!-- 表格开始 -->
    <el-table :data="pagingWaiters" border style="width: 100%">
...
export default {
  ...
  computed: {
    ...mapState("waiter", ["pagingWaiters", "total"]),
  },
  methods: {
    ...mapActions("waiter", [
      "findAll",
    ...
    submit() {
      ...
      // this.from.id=nanoid();
      if(this.title==="添加员工"){
        this.saveWaiter({user:this.form,isAdd:true});
      }else if(this.title==="修改员工信息"){
        this.saveWaiter({user:this.form,isAdd:false});
      }
      // 关闭模态框
